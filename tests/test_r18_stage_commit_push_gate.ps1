...
foreach ($assertion in @(
        ...
        @{ label = "R18 is active through R18-021 only after status updates"; script = { Test-R18StageCommitPushStatusTruth -RepositoryRoot $repoRoot } },
        @{ label = "R18-022 onward remain planned only"; script = { Test-R18StageCommitPushStatusTruth -RepositoryRoot $repoRoot } }
    )) {
    try {
        & $assertion.script
