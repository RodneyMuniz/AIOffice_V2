...
$module = Import-Module (Join-Path $repoRoot "tools\R16HandoffPacketGenerator.psm1") -Force -PassThru
$newReportObject = $module.ExportedCommands["New-R16HandoffPacketReportObject"]
...
$testReportObject = $module.ExportedCommands["Test-R16HandoffPacketReportObject"]
$testContract = $module.ExportedCommands["Test-R16HandoffPacketReportContract"]
$stableJson = $module.ExportedCommands["ConvertTo-StableJson"]
$newFixtures = $module.ExportedCommands["New-R16HandoffPacketFixtureFiles"]

$validPassed = 0
$invalidRejected = 0
$failures = @()
$fixtureRootRel = "tests\fixtures\r16_handoff_packet_generator"
$fixtureRoot = Join-Path $repoRoot $fixtureRootRel
$validFixtureRel = Join-Path $fixtureRootRel "valid_handoff_packet_report.json"
$validFixturePath = Join-Path $repoRoot $validFixtureRel
$stateReportRel = "state\workflow\r16_handoff_packet_report.json"
$tempRootRel = Join-Path "state\workflow" ("r16handoffpacket" + [guid]::NewGuid().ToString("N"))
...
function Assert-CompactMutationFixture {
    ...
    if ([string]$FixtureSpec.base_fixture -ne "valid_handoff_packet_report.json") {
        throw "$FixtureName base_fixture must be valid_handoff_packet_report.json."
    }
...
try {
    ...
    foreach ($requiredPath in @(
        "contracts\workflow\r16_handoff_packet_report.contract.json",
        "tools\R16HandoffPacketGenerator.psm1",
        "tools\new_r16_handoff_packets.ps1",
        "tools\validate_r16_handoff_packet_report.ps1",
        "tests\test_r16_handoff_packet_generator.ps1",
        $stateReportRel,
        $validFixtureRel
    )) {
    ...
    foreach ($forbiddenPath in @(
        ...
        if (Test-Path -LiteralPath (Join-Path $repoRoot $forbiddenPath)) {
            $failures += "FAIL forbidden R16-021 overbuild artifact exists: $forbiddenPath"
        }
    }

    $contractResult = & $testContract -RepositoryRoot $repoRoot
    if ($contractResult.SourceTask -ne "R16-021" -or $contractResult.DependencyRefCount -ne 8) {
        $failures += "FAIL contract: expected R16-021 handoff packet report contract with eight exact dependency refs."
    }
    else {
    ...
    if ((& $stableJson $first) -ne (& $stableJson $second)) {
        $failures += "FAIL determinism: two generated R16-021 handoff packet report objects differ after stable normalization."
    }
    else {
        Write-Output "PASS determinism: generated R16-021 handoff packet report object is stable across two generations."
        $validPassed += 1
    }

    $tempReportRel = Join-Path $tempRootRel "r16_handoff_packet_report.json"
    $generated = & $newReport -OutputPath $tempReportRel -RepositoryRoot $repoRoot
    if ($generated.ActiveThroughTask -ne "R16-021" -or $generated.PlannedTaskStart -ne "R16-022" -or $generated.PlannedTaskEnd -ne "R16-026" -or $generated.AggregateVerdict -ne "generated_handoff_packets_all_blocked_by_transition_gate" -or $generated.BlockedHandoffCount -ne 4 -or $generated.ExecutableHandoffCount -ne 0) {
        $failures += "FAIL generated report: expected active through R16-021, R16-022 through R16-026 planned only, four blocked handoff packets, zero executable handoffs, and generated_handoff_packets_all_blocked_by_transition_gate."
    }
    else {
        Write-Output ("PASS generated report: packets={0}, blocked={1}, executable={2}, verdict={3}" -f $generated.HandoffPacketCount, $generated.BlockedHandoffCount, $generated.ExecutableHandoffCount, $generated.AggregateVerdict)
        $validPassed += 1
    }
    ...
    if ($stateResult.ActiveThroughTask -ne "R16-021" -or $stateResult.PlannedTaskStart -ne "R16-022" -or $stateResult.PlannedTaskEnd -ne "R16-026" -or $stateResult.BlockedHandoffCount -ne 4 -or $stateResult.ExecutableHandoffCount -ne 0 -or $stateResult.BudgetGuardVerdict -ne "failed_closed_over_budget" -or $stateResult.EstimatedTokensUpperBound -ne $expectedEstimatedUpperBound -or $stateResult.MaxEstimatedTokensUpperBound -ne $expectedMaxUpperBound) {
        $failures += ("FAIL committed R16-021 report: expected failed_closed_over_budget guard values {0} over {1}, all handoffs blocked, and zero executable handoffs." -f $expectedEstimatedUpperBound, $expectedMaxUpperBound)
    }
    else {
        Write-Output ("PASS committed R16-021 report: blocked={0}, executable={1}, verdict={2}" -f $stateResult.BlockedHandoffCount, $stateResult.ExecutableHandoffCount, $stateResult.AggregateVerdict)
        $validPassed += 1
    }

    $validFixtureResult = & $testReport -Path $validFixtureRel -RepositoryRoot $repoRoot
    if ($validFixtureResult.BlockedHandoffCount -ne 4 -or $validFixtureResult.ExecutableHandoffCount -ne 0 -or $validFixtureResult.AggregateVerdict -ne "generated_handoff_packets_all_blocked_by_transition_gate") {
        $failures += "FAIL valid fixture: expected all generated handoff packets blocked with zero executable handoffs."
    }
    else {
    ...
    $expectedInvalidFragments = [ordered]@{
        "invalid_missing_required_top_level_field.json" = @("missing required field 'generation_mode'")
        ...
        "invalid_missing_handoff_packets.json" = @("missing required field 'handoff_packets'")
        "invalid_missing_source_role_id.json" = @("missing required field 'source_role_id'")
        "invalid_missing_target_role_id.json" = @("missing required field 'target_role_id'")
        "invalid_missing_transition_decision.json" = @("missing required field 'transition_decision'")
        "invalid_handoff_executable_transition_gate_blocked.json" = @("executable while transition gate is blocked")
        "invalid_handoff_executable_guard_failed_closed_over_budget.json" = @("guard is failed_closed_over_budget")
        "invalid_blocked_reason_missing_failed_closed_over_budget.json" = @("blocked_reason", "failed_closed_over_budget")
        "invalid_missing_card_state_ref.json" = @("missing required field 'card_state_ref'")
        "invalid_missing_memory_pack_ref.json" = @("missing required field 'memory_pack_ref'")
        "invalid_missing_context_load_plan_ref.json" = @("missing required field 'context_load_plan_ref'")
        "invalid_missing_context_budget_estimate_ref.json" = @("missing required field 'context_budget_estimate_ref'")
        "invalid_missing_packet_context_budget_guard_ref.json" = @("missing required field 'context_budget_guard_ref'")
        "invalid_missing_evidence_refs.json" = @("missing required field 'evidence_refs'")
        ...
        "invalid_r16_022_implementation_claim.json" = @("R16-022 implementation claim")
    ...
    $failures += ("FAIL R16 handoff packet generator harness: {0}" -f $_.Exception.Message)
}
finally {
...
if ($failures.Count -gt 0) {
    ...
    throw ("R16 handoff packet generator tests failed. Valid passed: {0}. Invalid rejected: {1}. Failures: {2}" -f $validPassed, $invalidRejected, $failures.Count)
}

Write-Output ("All R16 handoff packet generator tests passed. Valid passed: {0}. Invalid rejected: {1}." -f $validPassed, $invalidRejected)
