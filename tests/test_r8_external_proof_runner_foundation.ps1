...
try {
    ...
    if ($activeStateText -notmatch 'R8-001` through `R8-007` complete and `R8-008` through `R8-009` planned') {
        $failures += "FAIL ACTIVE_STATE does not mark only R8-007 complete with R8-008 and R8-009 still planned."
    }
    ...
    if ($kanbanText -notmatch '### `R8-008` Add status-doc gating[\s\S]*?- Status: planned') {
        $failures += "FAIL KANBAN does not keep R8-008 planned."
    }
    ...
    if ($r8AuthorityText -notmatch '`R8-001` through `R8-007` are complete in repo truth\.\s+`R8-008` through `R8-009` are planned only\.') {
        $failures += "FAIL R8 authority document does not preserve the R8-007 done / R8-008 and R8-009 planned boundary."
    }
    ...
    if ($readmeText -notmatch '`R8-008` through `R8-009` remain planned only') {
        $failures += "FAIL README does not keep R8-008 and R8-009 planned only."
    }
