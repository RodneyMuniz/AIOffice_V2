...
try {
    ...
    if ($activeStateText -notmatch 'R8-001` through `R8-009` complete') {
        $failures += "FAIL ACTIVE_STATE does not mark R8-001 through R8-009 complete after closeout."
    }
    ...
    if ($kanbanText -notmatch '### `R8-009` Pilot and close R8 narrowly[\s\S]*?- Status: done') {
        $failures += "FAIL KANBAN does not mark R8-009 done after closeout."
    }
    if ($r8AuthorityText -notmatch '`R8-001` through `R8-009` are complete in repo truth\.') {
        $failures += "FAIL R8 authority document does not record R8-009 completion."
    }
    if ($r8AuthorityText -notmatch 'external proof runner foundation exists' -or $r8AuthorityText -notmatch 'no concrete CI or external proof artifact is claimed') {
        $failures += "FAIL R8 authority document does not preserve the external-proof foundation-only non-claim."
    }
    if ($readmeText -notmatch '`R8-007` is complete') {
        $failures += "FAIL README does not reflect R8-007 completion."
    }
    if ($readmeText -notmatch '`R8-008` is complete' -or $readmeText -notmatch '`R8-009` is complete') {
        $failures += "FAIL README does not record the R8-009 closeout boundary."
    }
    if ($readmeText -notmatch 'no concrete CI or external proof artifact is claimed') {
        $failures += "FAIL README does not preserve the no-external-proof-artifact non-claim."
    }
