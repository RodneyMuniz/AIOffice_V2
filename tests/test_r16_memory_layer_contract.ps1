...
try {
    ...
    if (-not $status.R16Opened -or $status.R16DoneThrough -ne 15 -or $status.R16PlannedStart -ne 16 -or $status.R16PlannedThrough -ne 26) {
        $failures += "FAIL status posture: expected R16 active through R16-015 only with R16-016 through R16-026 planned only."
    }
    else {
        Write-Output "PASS status posture: R16 active through R16-015 only; R16-016 through R16-026 remain planned only."
        $validPassed += 1
    }
