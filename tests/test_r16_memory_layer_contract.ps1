...
try {
    ...
    if (-not $status.R16Opened -or $status.R16DoneThrough -ne 21 -or $status.R16PlannedStart -ne 22 -or $status.R16PlannedThrough -ne 26) {
        $failures += "FAIL status posture: expected R16 active through R16-021 only with R16-022 through R16-026 planned only."
    }
    else {
        Write-Output "PASS status posture: R16 active through R16-021 only; R16-022 through R16-026 remain planned only."
        $validPassed += 1
    }
