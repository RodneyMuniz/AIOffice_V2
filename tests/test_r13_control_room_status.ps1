...
try {
    ...
    if ($statusValidation.CompletedTaskCount -ne 13 -or $statusValidation.PlannedTaskCount -ne 5 -or $statusValidation.NextLegalAction -ne "R13-014") {
        $failures += "FAIL generated status: status did not preserve R13-013 boundary."
    }
    else {
        Write-Output "PASS generated status: R13-013 boundary validates."
        $validPassed += 1
    }
    ...
    if (-not (Test-Path -LiteralPath $generatedManifestPath)) {
        ...
        if ($manifestText -notmatch "Stale-state checks passed: ``True``" -or $manifestText -notmatch "R13-014") {
    ...
    if (($completedTaskIds -join "|") -ne ((1..13 | ForEach-Object { "R13-{0}" -f $_.ToString("000") }) -join "|")) {
        $failures += "FAIL generated status: completed tasks are not R13-001 through R13-013 only."
    }
    else {
        Write-Output "PASS generated status: R13 active through R13-013 only."
        $validPassed += 1
    }
    if (($plannedTaskIds -join "|") -ne ((14..18 | ForEach-Object { "R13-{0}" -f $_.ToString("000") }) -join "|")) {
        $failures += "FAIL generated status: planned tasks are not R13-014 through R13-018 only."
    }
    else {
        Write-Output "PASS generated status: R13-014 through R13-018 planned only."
        $validPassed += 1
    }
