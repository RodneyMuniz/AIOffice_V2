...
try {
    ...
    if ($statusSnapshot.DoneThrough -ne 2 -or $statusSnapshot.PlannedStart -ne 3 -or $statusSnapshot.PlannedThrough -ne 26) {
        $failures += "FAIL status posture: expected R16 active through R16-002 only with R16-003 through R16-026 planned only."
    }
    else {
        Write-Output "PASS status posture: R16 active through R16-002 only; R16-003 through R16-026 remain planned only."
        $validPassed += 1
    }
