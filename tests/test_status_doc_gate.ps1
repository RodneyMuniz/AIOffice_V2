...
try {
    ...
    Invoke-ExpectedRefusal -Label "r16-004-implementation-overclaim" -RequiredFragments @("R16-004 implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r16-004-implementation")
        Add-Content -LiteralPath $scenario.R16AuthorityPath -Value ($crlf + "R16-004 implementation is complete.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
