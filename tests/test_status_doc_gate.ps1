...
try {
    ...
    Invoke-ExpectedRefusal -Label "r15-implementation-beyond-r15-005" -RequiredFragments @("R15 implementation beyond R15-005") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r15-implementation")
        Add-Content -LiteralPath $scenario.R15AuthorityPath -Value ($crlf + "R15-006 RACI matrix is implemented and complete.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
