...
try {
    ...
    Invoke-ExpectedRefusal -Label "r15-implementation-beyond-r15-007" -RequiredFragments @("R15 implementation beyond R15-007") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r15-implementation")
        Add-Content -LiteralPath $scenario.R15AuthorityPath -Value ($crlf + "R15-008 dry run is executed and complete.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
