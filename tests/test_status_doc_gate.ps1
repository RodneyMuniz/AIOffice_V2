...
try {
    ...
    Invoke-ExpectedRefusal -Label "r16-023-implementation-overclaim" -RequiredFragments @("R16-023 or later implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r16-023-implementation")
        Add-Content -LiteralPath $scenario.R16AuthorityPath -Value ($crlf + "R16-023 implementation is complete.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
    ...
    Invoke-ExpectedRefusal -Label "r16-role-handoff-drill-artifact-overclaim" -RequiredFragments @("role-handoff drill artifact") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r16-role-handoff-drill")
        Add-Content -LiteralPath $scenario.R16AuthorityPath -Value ($crlf + "The R16 role-handoff drill exists for PM to Developer to QA.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }

    Invoke-ExpectedRefusal -Label "r16-audit-readiness-drill-artifact-overclaim" -RequiredFragments @("audit-readiness drill artifact") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r16-audit-readiness-drill")
        Add-Content -LiteralPath $scenario.R16AuthorityPath -Value ($crlf + "The R16 audit-readiness drill exists for evidence inspection.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }

    Invoke-ExpectedRefusal -Label "r16-friction-metrics-artifact-overclaim" -RequiredFragments @("friction metrics artifact") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r16-friction-metrics")
        Add-Content -LiteralPath $scenario.R16AuthorityPath -Value ($crlf + "The R16 friction metrics report exists for closeout.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
