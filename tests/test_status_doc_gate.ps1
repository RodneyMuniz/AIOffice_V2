...
try {
    ...
    Invoke-ExpectedRefusal -Label "r17-022-implemented-overclaim" -RequiredFragments @("R17-022 or later implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-022-implemented")
        Add-Content -LiteralPath $scenario.ReadmePath -Value ($crlf + "R17-022 is implemented by this pass.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
