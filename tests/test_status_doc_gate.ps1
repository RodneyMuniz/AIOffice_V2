...
try {
    ...
    Invoke-ExpectedRefusal -Label "r17-021-implemented-overclaim" -RequiredFragments @("R17-021 or later implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-021-implemented")
        Add-Content -LiteralPath $scenario.ReadmePath -Value ($crlf + "R17-021 is implemented by this pass.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
