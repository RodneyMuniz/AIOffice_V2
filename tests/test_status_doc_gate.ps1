...
try {
    ...
    Invoke-ExpectedRefusal -Label "r17-007-implemented-overclaim" -RequiredFragments @("R17-007 or later implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-007-implemented")
        Add-Content -LiteralPath $scenario.ReadmePath -Value ($crlf + "R17-007 is implemented by this pass.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
