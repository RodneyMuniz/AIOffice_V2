...
try {
    ...
    Invoke-ExpectedRefusal -Label "r17-006-implemented-overclaim" -RequiredFragments @("R17-006 or later implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-006-implemented")
        Add-Content -LiteralPath $scenario.ReadmePath -Value ($crlf + "R17-006 is implemented by this pass.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
