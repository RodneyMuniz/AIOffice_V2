...
try {
    ...
    Invoke-ExpectedRefusal -Label "r16-007-implementation-overclaim" -RequiredFragments @("R16-007 or later implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r16-007-implementation")
        Add-Content -LiteralPath $scenario.R16AuthorityPath -Value ($crlf + "R16-007 implementation is complete.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
    ...
    Invoke-ExpectedRefusal -Label "r16-generated-role-memory-pack-overclaim" -RequiredFragments @("generated role memory packs") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r16-role-memory-pack")
        Add-Content -LiteralPath $scenario.R16AuthorityPath -Value ($crlf + "R16 generated role memory packs for PM and Developer.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
