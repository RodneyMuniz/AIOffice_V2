...
try {
    ...
    Invoke-ExpectedRefusal -Label "r17-025-implemented-overclaim" -RequiredFragments @("R17-025 or later implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-025-implemented")
        Add-Content -LiteralPath $scenario.ReadmePath -Value ($crlf + "R17-025 is implemented by this pass.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
