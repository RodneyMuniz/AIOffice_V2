...
try {
    ...
    Invoke-ExpectedRefusal -Label "r17-028-live-runtime-overclaim" -RequiredFragments @("R17-028 closeout/runtime overclaim") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-028-live-runtime")
        Add-Content -LiteralPath $scenario.ReadmePath -Value ($crlf + "R17-028 implements live recovery-loop runtime.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
    ...
    Invoke-ExpectedRefusal -Label "r17-dev-output-overclaim" -RequiredFragments @("real Dev output") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-dev-output")
        Add-Content -LiteralPath $scenario.R17AuthorityPath -Value ($crlf + "Dev output exists for R17-007.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }

    Invoke-ExpectedRefusal -Label "r17-qa-result-overclaim" -RequiredFragments @("real QA result") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-qa-result")
        Add-Content -LiteralPath $scenario.R17AuthorityPath -Value ($crlf + "QA result passed for R17-007.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }

    Invoke-ExpectedRefusal -Label "r17-audit-verdict-overclaim" -RequiredFragments @("real audit verdict") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-audit-verdict")
        Add-Content -LiteralPath $scenario.R17AuthorityPath -Value ($crlf + "Audit verdict accepted for R17-007.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }

    Invoke-ExpectedRefusal -Label "r17-kanban-product-runtime-overclaim" -RequiredFragments @("product runtime") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-kanban-product-runtime")
        Add-Content -LiteralPath $scenario.KanbanPath -Value ($crlf + "Kanban product runtime is working for R17.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }

    Invoke-ExpectedRefusal -Label "r17-live-board-mutation-overclaim" -RequiredFragments @("live board mutation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-live-board-mutation")
        Add-Content -LiteralPath $scenario.KanbanPath -Value ($crlf + "Live board mutation is implemented for R17.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }

    Invoke-ExpectedRefusal -Label "r17-runtime-card-creation-overclaim" -RequiredFragments @("runtime card creation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-runtime-card-creation")
        Add-Content -LiteralPath $scenario.KanbanPath -Value ($crlf + "Runtime card creation is implemented for R17.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }

    Invoke-ExpectedRefusal -Label "r17-orchestrator-runtime-overclaim" -RequiredFragments @("Orchestrator runtime") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-orchestrator-runtime")
        Add-Content -LiteralPath $scenario.R17AuthorityPath -Value ($crlf + "Orchestrator runtime is working for R17.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }

    Invoke-ExpectedRefusal -Label "r17-executable-handoff-overclaim" -RequiredFragments @("executable handoff or transition") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-executable-handoff")
        Add-Content -LiteralPath $scenario.R17AuthorityPath -Value ($crlf + "Executable handoffs are implemented for R17.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
