...
try {
    ...
    Invoke-ExpectedRefusal -Label "r17-008-implemented-overclaim" -RequiredFragments @("R17-008 or later implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-008-implemented")
        Add-Content -LiteralPath $scenario.ReadmePath -Value ($crlf + "R17-008 is implemented by this pass.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
