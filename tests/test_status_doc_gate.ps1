...
try {
    ...
    Invoke-ExpectedRefusal -Label "r16-019-implementation-overclaim" -RequiredFragments @("R16-019 or later implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r16-019-implementation")
        Add-Content -LiteralPath $scenario.R16AuthorityPath -Value ($crlf + "R16-019 implementation is complete.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
    ...
    Invoke-ExpectedRefusal -Label "r16-role-run-envelope-overclaim" -RequiredFragments @("generated role-run envelope or handoff packet implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r16-role-run-envelope")
        Add-Content -LiteralPath $scenario.R16AuthorityPath -Value ($crlf + "Generated role-run envelopes exist for R16.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }

    Invoke-ExpectedRefusal -Label "r16-handoff-packet-overclaim" -RequiredFragments @("generated role-run envelope or handoff packet implementation") -Action {
