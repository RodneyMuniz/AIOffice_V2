...
try {
    ...
    Invoke-ExpectedRefusal -Label "r16-003-implementation-overclaim" -RequiredFragments @("R16-003 implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r16-003-implementation")
        Add-Content -LiteralPath $scenario.R16AuthorityPath -Value ($crlf + "R16-003 implementation is complete.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
