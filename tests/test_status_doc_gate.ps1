...
try {
    ...
    if ($liveValidation.ActiveMilestone -ne "R18 Automated Recovery Runtime and API Orchestration" -or -not $liveValidation.R18Opened -or $liveValidation.R18DoneThrough -ne 27 -or $liveValidation.R18PlannedStart -ne 28 -or $liveValidation.R18PlannedThrough -ne 28 -or -not $liveValidation.R17Closed -or $liveValidation.R17DoneThrough -ne 28) {
        $failures += "FAIL valid: live status-doc gate did not report R17 closed and R18 active through R18-027 only."
    }
    else {
        Write-Output "PASS valid: live status-doc gate reports R17 closed with caveats and R18 active through R18-027 only."
        $validPassed += 1
    }
}
catch {
...
Invoke-ExpectedRefusal -Label "r18-028-done-in-kanban" -Expected "does not match KANBAN" -Mutation {
    param($root)
