...
try {
    ...
    Invoke-ExpectedRefusal -Label "r17-028-implemented-overclaim" -RequiredFragments @("R17-028 or later implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-028-implemented")
        Add-Content -LiteralPath $scenario.ReadmePath -Value ($crlf + "R17-028 is implemented by this pass.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
