...
try {
    ...
    Invoke-ExpectedRefusal -Label "r17-016-implemented-overclaim" -RequiredFragments @("R17-016 or later implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r17-016-implemented")
        Add-Content -LiteralPath $scenario.ReadmePath -Value ($crlf + "R17-016 is implemented by this pass.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
