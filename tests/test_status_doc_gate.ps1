...
try {
    ...
    Invoke-ExpectedRefusal -Label "r16-006-implementation-overclaim" -RequiredFragments @("R16-006 or later implementation") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r16-006-implementation")
        Add-Content -LiteralPath $scenario.R16AuthorityPath -Value ($crlf + "R16-006 implementation is complete.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
    ...
    Invoke-ExpectedRefusal -Label "r16-role-specific-memory-pack-overclaim" -RequiredFragments @("role-specific memory packs") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r16-role-memory-pack")
        Add-Content -LiteralPath $scenario.R16AuthorityPath -Value ($crlf + "R16 implements role-specific memory packs for PM and Developer.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
