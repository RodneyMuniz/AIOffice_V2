...
try {
    ...
    Invoke-ExpectedRefusal -Label "r16-026-implementation-overclaim" -RequiredFragments @("R16-026 implementation or final proof package") -Action {
        $scenario = New-StatusDocHarness -Root (Join-Path $tempRoot "invalid-r16-026-implementation")
        Add-Content -LiteralPath $scenario.R16AuthorityPath -Value ($crlf + "R16-026 final proof package implementation is complete.") -Encoding UTF8
        & $testStatusDocGate -RepositoryRoot $scenario.Root | Out-Null
    }
