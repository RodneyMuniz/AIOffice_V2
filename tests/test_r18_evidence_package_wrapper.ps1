...
foreach ($assertion in @(
        ...
        @{ label = "R18 is active through R18-020 only after status updates"; script = { Test-R18EvidencePackageWrapperStatusTruth -RepositoryRoot $repoRoot | Out-Null } },
        @{ label = "R18-021 onward remain planned only"; script = { Test-R18EvidencePackageWrapperStatusTruth -RepositoryRoot $repoRoot | Out-Null } }
    )) {
    try {
        & $assertion.script
