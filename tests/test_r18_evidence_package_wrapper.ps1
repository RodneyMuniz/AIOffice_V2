...
foreach ($assertion in @(
        ...
        @{ label = "R18 is active through R18-028 only after status updates"; script = { Test-R18EvidencePackageWrapperStatusTruth -RepositoryRoot $repoRoot | Out-Null } },
        @{ label = "R18-028 final package boundary is preserved"; script = { Test-R18EvidencePackageWrapperStatusTruth -RepositoryRoot $repoRoot | Out-Null } }
    )) {
    try {
        & $assertion.script
