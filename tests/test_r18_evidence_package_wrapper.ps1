...
foreach ($assertion in @(
        ...
        @{ label = "R18 is active through R18-027 only after status updates"; script = { Test-R18EvidencePackageWrapperStatusTruth -RepositoryRoot $repoRoot | Out-Null } },
        @{ label = "R18-028 remains planned only"; script = { Test-R18EvidencePackageWrapperStatusTruth -RepositoryRoot $repoRoot | Out-Null } }
    )) {
    try {
        & $assertion.script
