...
foreach ($assertion in @(
        ...
        @{ label = "R18 is active through R18-019 only after status updates"; script = { Test-R18StatusDocGateWrapperStatusTruth -RepositoryRoot $repoRoot } },
        @{ label = "R18-020 onward remain planned only"; script = { Test-R18StatusDocGateWrapperStatusTruth -RepositoryRoot $repoRoot } }
    )) {
    try {
        & $assertion.script
