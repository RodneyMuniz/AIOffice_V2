...
$module = Import-Module (Join-Path $repoRoot "tools\R16RestartCompactionRecoveryDrill.psm1") -Force -PassThru
$newReportObject = $module.ExportedCommands["New-R16RestartCompactionRecoveryDrillObject"]
...
$testReportObject = $module.ExportedCommands["Test-R16RestartCompactionRecoveryDrillObject"]
$testContract = $module.ExportedCommands["Test-R16RestartCompactionRecoveryDrillContract"]
$stableJson = $module.ExportedCommands["ConvertTo-StableJson"]
$newFixtures = $module.ExportedCommands["New-R16RestartCompactionRecoveryDrillFixtureFiles"]

$validPassed = 0
$invalidRejected = 0
$failures = @()
$fixtureRootRel = "tests\fixtures\r16_restart_compaction_recovery_drill"
$fixtureRoot = Join-Path $repoRoot $fixtureRootRel
$validFixtureRel = Join-Path $fixtureRootRel "valid_restart_compaction_recovery_drill.json"
$validFixturePath = Join-Path $repoRoot $validFixtureRel
$stateReportRel = "state\workflow\r16_restart_compaction_recovery_drill.json"
$tempRootRel = Join-Path "state\workflow" ("r16restartdrill" + [guid]::NewGuid().ToString("N"))
...
function Assert-CompactMutationFixture {
    ...
    if ([string]$FixtureSpec.base_fixture -ne "valid_restart_compaction_recovery_drill.json") {
        throw "$FixtureName base_fixture must be valid_restart_compaction_recovery_drill.json."
    }
...
try {
    ...
    foreach ($requiredPath in @(
        "contracts\workflow\r16_restart_compaction_recovery_drill.contract.json",
        "tools\R16RestartCompactionRecoveryDrill.psm1",
        "tools\new_r16_restart_compaction_recovery_drill.ps1",
        "tools\validate_r16_restart_compaction_recovery_drill.ps1",
        "tests\test_r16_restart_compaction_recovery_drill.ps1",
        $stateReportRel,
        $validFixtureRel
    )) {
    ...
    foreach ($forbiddenPath in @(
        ...
        if (Test-Path -LiteralPath (Join-Path $repoRoot $forbiddenPath)) {
            $failures += "FAIL forbidden R16-022 overbuild artifact exists: $forbiddenPath"
        }
    }

    $contractResult = & $testContract -RepositoryRoot $repoRoot
    if ($contractResult.SourceTask -ne "R16-022" -or $contractResult.DependencyRefCount -ne 11) {
        $failures += "FAIL contract: expected R16-022 restart/compaction recovery drill contract with eleven exact dependency refs."
    }
    else {
    ...
    if ((& $stableJson $first) -ne (& $stableJson $second)) {
        $failures += "FAIL determinism: two generated R16-022 restart/compaction recovery drill objects differ after stable normalization."
    }
    else {
        Write-Output "PASS determinism: generated R16-022 restart/compaction recovery drill object is stable across two generations."
        $validPassed += 1
    }

    $tempReportRel = Join-Path $tempRootRel "r16_restart_compaction_recovery_drill.json"
    $generated = & $newReport -OutputPath $tempReportRel -RepositoryRoot $repoRoot
    if ($generated.ActiveThroughTask -ne "R16-022" -or $generated.PlannedTaskStart -ne "R16-023" -or $generated.PlannedTaskEnd -ne "R16-026" -or $generated.AggregateVerdict -ne "passed_bounded_restart_recovery_drill_with_blocked_execution" -or $generated.ExactRecoveryInputCount -ne 11 -or $generated.BlockedHandoffCount -ne 4 -or $generated.ExecutableHandoffCount -ne 0) {
        $failures += "FAIL generated report: expected active through R16-022, R16-023 through R16-026 planned only, eleven exact recovery inputs, four blocked handoffs, zero executable handoffs, and passed_bounded_restart_recovery_drill_with_blocked_execution."
    }
    else {
        Write-Output ("PASS generated report: exact_inputs={0}, blocked_handoffs={1}, executable_handoffs={2}, verdict={3}" -f $generated.ExactRecoveryInputCount, $generated.BlockedHandoffCount, $generated.ExecutableHandoffCount, $generated.AggregateVerdict)
        $validPassed += 1
    }
    ...
    if ($stateResult.ActiveThroughTask -ne "R16-022" -or $stateResult.PlannedTaskStart -ne "R16-023" -or $stateResult.PlannedTaskEnd -ne "R16-026" -or $stateResult.ExactRecoveryInputCount -ne 11 -or $stateResult.BlockedHandoffCount -ne 4 -or $stateResult.ExecutableHandoffCount -ne 0 -or $stateResult.GuardVerdict -ne "failed_closed_over_budget" -or $stateResult.EstimatedTokensUpperBound -ne $expectedUpperBound -or $stateResult.Threshold -ne $expectedThreshold) {
        $failures += ("FAIL state R16-022 report: expected failed_closed_over_budget guard values {0} over {1}, exact input count 11, all handoffs blocked, and zero executable handoffs." -f $expectedUpperBound, $expectedThreshold)
    }
    else {
        Write-Output ("PASS state report: exact_inputs={0}, blocked_handoffs={1}, executable_handoffs={2}, verdict={3}" -f $stateResult.ExactRecoveryInputCount, $stateResult.BlockedHandoffCount, $stateResult.ExecutableHandoffCount, $stateResult.AggregateVerdict)
        $validPassed += 1
    }

    $validFixtureResult = & $testReport -Path $validFixtureRel -RepositoryRoot $repoRoot
    if ($validFixtureResult.ExactRecoveryInputCount -ne 11 -or $validFixtureResult.BlockedHandoffCount -ne 4 -or $validFixtureResult.ExecutableHandoffCount -ne 0 -or $validFixtureResult.AggregateVerdict -ne "passed_bounded_restart_recovery_drill_with_blocked_execution") {
        $failures += "FAIL valid fixture: expected bounded restart recovery drill with blocked execution posture."
    }
    else {
    ...
    $expectedInvalidFragments = [ordered]@{
        "invalid_missing_required_top_level_field.json" = @("missing required field 'restart_scenario'")
        ...
        "invalid_recovered_posture_r16_023_complete.json" = @("R16-023 complete claim")
        "invalid_raw_chat_history_canonical_state.json" = @("raw chat history as canonical state")
        ...
        "invalid_r16_023_implementation_claim.json" = @("R16-023 implementation claim")
    ...
    $failures += ("FAIL R16 restart/compaction recovery drill harness: {0}" -f $_.Exception.Message)
}
finally {
...
if ($failures.Count -gt 0) {
    ...
    throw ("R16 restart/compaction recovery drill tests failed. Valid passed: {0}. Invalid rejected: {1}. Failures: {2}" -f $validPassed, $invalidRejected, $failures.Count)
}

Write-Output ("All R16 restart/compaction recovery drill tests passed. Valid passed: {0}. Invalid rejected: {1}." -f $validPassed, $invalidRejected)
