...
foreach ($assertion in @(
        ...
        @{ label = "R18 is active through R18-016 only after status updates"; script = { Test-R18OperatorApprovalStatusTruth -RepositoryRoot $repoRoot } },
        @{ label = "R18-017 onward remain planned only"; script = { Test-R18OperatorApprovalStatusTruth -RepositoryRoot $repoRoot } }
    )) {
    try {
        & $assertion.script
