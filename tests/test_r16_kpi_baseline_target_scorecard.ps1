...
try {
    ...
    if (-not $status.R16Opened -or $status.R16DoneThrough -ne 20 -or $status.R16PlannedStart -ne 21 -or $status.R16PlannedThrough -ne 26) {
        $failures += "FAIL status posture: expected R16 active through R16-020 only with R16-021 through R16-026 planned only."
    }
    else {
        Write-Output "PASS status posture: R16 active through R16-020 only; R16-021 through R16-026 remain planned only."
        $validPassed += 1
    }
