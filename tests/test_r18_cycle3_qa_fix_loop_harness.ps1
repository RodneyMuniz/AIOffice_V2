...
foreach ($assertion in @(
        ...
        @{ label = "R18 is active through R18-025 only after status updates"; script = { Test-R18Cycle3QaFixLoopHarnessStatusTruth -RepositoryRoot $repoRoot | Out-Null } },
        @{ label = "R18-026 onward remain planned only"; script = { Test-R18Cycle3QaFixLoopHarnessStatusTruth -RepositoryRoot $repoRoot | Out-Null } }
    )) {
    try {
        & $assertion.script
