...
foreach ($assertion in @(
        ...
        @{ label = "R18 is active through R18-022 only after status updates"; script = { Test-R18BoardCardEventModelStatusTruth -RepositoryRoot $repoRoot | Out-Null } },
        @{ label = "R18-023 onward remain planned only"; script = { Test-R18BoardCardEventModelStatusTruth -RepositoryRoot $repoRoot | Out-Null } }
    )) {
    try {
        & $assertion.script
