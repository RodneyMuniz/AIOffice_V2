...
foreach ($assertion in @(
        ...
        @{ label = "R18 is active through R18-024 only after status updates"; script = { Test-R18CompactFailureRecoveryDrillStatusTruth -RepositoryRoot $repoRoot | Out-Null } },
        @{ label = "R18-025 onward remain planned only"; script = { Test-R18CompactFailureRecoveryDrillStatusTruth -RepositoryRoot $repoRoot | Out-Null } }
    )) {
    try {
        & $assertion.script
