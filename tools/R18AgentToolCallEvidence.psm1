...
function Test-R18AgentToolCallEvidenceStatusTruth {
    ...
    foreach ($taskNumber in 1..28) {
        ...
        if ($taskNumber -le 23) {
            Assert-R18AgentToolCallEvidenceCondition -Condition ($authorityStatuses[$taskId] -eq "done") -Message "$taskId must be done after R18-023."
        }
        else {
            Assert-R18AgentToolCallEvidenceCondition -Condition ($authorityStatuses[$taskId] -eq "planned") -Message "$taskId must remain planned only after R18-023."
        }
    }
