...
function Test-R18StageCommitPushStatusTruth {
    ...
    foreach ($taskNumber in 1..28) {
        ...
        if ($taskNumber -le 21) {
            Assert-R18StageCommitPushCondition -Condition ($authorityStatuses[$taskId] -eq "done") -Message "$taskId must be done after R18-021."
        }
        else {
            Assert-R18StageCommitPushCondition -Condition ($authorityStatuses[$taskId] -eq "planned") -Message "$taskId must remain planned only after R18-021."
        }
    }
