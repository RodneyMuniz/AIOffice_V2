...
function Test-R9ClosedStatus {
    ...
    if ($AllowR10Closed -and $AllowR11Closed -and $AllowR12Active) {
        Assert-RegexMatch -Text $Texts.ActiveState -Pattern '## Active Milestone\s+`R12 External API Runner, Actionable QA, and Operator Control-Room Workflow Pilot`\s+is now active in repo truth through `R12-010` only\.' -Message "ACTIVE_STATE must declare R12 as active through R12-010 only."
...
function Test-R11OpeningStatus {
    ...
    if ($Closed -and $AllowR12Active) {
        Assert-RegexMatch -Text $Texts.Readme -Pattern 'R12 External API Runner, Actionable QA, and Operator Control-Room Workflow Pilot`\s+is now the active milestone in repo truth through `R12-010` only' -Message "README must declare R12 active through R12-010 only."
        Assert-RegexMatch -Text $Texts.ActiveState -Pattern '## Active Milestone\s+`R12 External API Runner, Actionable QA, and Operator Control-Room Workflow Pilot`\s+is now active in repo truth through `R12-010` only\.' -Message "ACTIVE_STATE must declare R12 active through R12-010 only."
        Assert-RegexMatch -Text $Texts.Kanban -Pattern '## Active Milestone\s+`R12 External API Runner, Actionable QA, and Operator Control-Room Workflow Pilot`' -Message "KANBAN must declare R12 as the active milestone."
...
function Test-R12OpeningStatus {
    ...
    if ($kanbanSnapshot.DoneThrough -ne 10 -or $kanbanSnapshot.PlannedStart -ne 11 -or $kanbanSnapshot.PlannedThrough -ne 21) {
        throw "R12 open status must keep only R12-001 through R12-010 done and R12-011 through R12-021 planned."
    }
    ...
    Assert-RegexMatch -Text $combinedText -Pattern '(?i)R12-011` through `R12-021` remain planned only|R12-011` through `R12-021` remain planned only' -Message "Status docs must preserve that R12-011 through R12-021 are planned only."
