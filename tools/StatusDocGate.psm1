...
function Test-R16OpeningStatus {
    ...
    if ($kanbanSnapshot.DoneThrough -ne 11 -or $kanbanSnapshot.PlannedStart -ne 12 -or $kanbanSnapshot.PlannedThrough -ne 26) {
        throw "R16 status must keep R16 active through R16-011 only with R16-012 through R16-026 planned only."
    }
    ...
    Assert-RegexMatch -Text $Texts.Readme -Pattern '`R16 Operational Memory, Artifact Map, and Role-Bound Workflow Foundation`\s+is now active on branch `release/r16-operational-memory-artifact-map-role-workflow-foundation` through `R16-011` only' -Message "README must declare R16 active on the R16 branch through R16-011 only."
    Assert-RegexMatch -Text $Texts.ActiveState -Pattern '## Active Milestone\s+`R16 Operational Memory, Artifact Map, and Role-Bound Workflow Foundation`\s+is now active in repo truth through `R16-011` only\.' -Message "ACTIVE_STATE must declare R16 as the active milestone through R16-011 only."
    ...
    Assert-RegexMatch -Text $Texts.R16Authority -Pattern '\*\*Milestone status:\*\*\s+Active in repo truth through `R16-011` only' -Message "R16 authority must declare R16 active through R16-011 only."
    ...
    Assert-RegexMatch -Text $Texts.R16Authority -Pattern 'R16-012` through `R16-026` remain planned only' -Message "R16 authority must keep R16-012 through R16-026 planned only."
    ...
    Assert-RegexMatch -Text $r16CurrentText -Pattern 'No audit maps are implemented yet|No audit map exists yet|No generated audit map exists yet' -Message "Status docs must state that no audit maps are implemented yet."
    Assert-RegexMatch -Text $r16CurrentText -Pattern 'No generated audit map exists yet' -Message "Status docs must state that no generated audit map exists yet."
    Assert-RegexMatch -Text $r16CurrentText -Pattern 'No audit map generator exists yet' -Message "Status docs must state that no audit map generator exists yet."
    Assert-RegexMatch -Text $r16CurrentText -Pattern 'No R15/R16 audit map exists yet' -Message "Status docs must state that no R15/R16 audit map exists yet."
    Assert-RegexMatch -Text $r16CurrentText -Pattern 'No context-load planner is implemented yet|No context-load planners are implemented yet|No context-load planner exists yet' -Message "Status docs must state that no context-load planner is implemented yet."
    Assert-RegexMatch -Text $r16CurrentText -Pattern 'No role-run envelopes are implemented yet|No role-run envelope is implemented yet|No role-run envelopes exist yet|No role-run envelope exists yet' -Message "Status docs must state that no role-run envelopes are implemented yet."
    Assert-RegexMatch -Text $r16CurrentText -Pattern 'No handoff packets are implemented yet|No handoff packet is implemented yet|No handoff packets exist yet|No handoff packet exists yet' -Message "Status docs must state that no handoff packet is implemented yet."
    ...
    Assert-NoForbiddenPositiveClaim -Text $r16CurrentText -Context "Status docs" -ClaimLabel "R16-012 or later implementation" -Pattern '(?i)\bR16-(0(?:1[2-9]|2[0-6]))\b.{0,160}\b(done|complete|completed|implemented|executed|ran|claimed|created)\b'
    ...
    Assert-NoForbiddenPositiveClaim -Text $r16CurrentText -Context "Status docs" -ClaimLabel "artifact map runtime or audit map implementation" -Pattern '(?i)\b(artifact map runtime|artifact maps runtime|operational artifact map runtime|audit map(?! contract)|audit maps(?! contract)|audit map runtime|audit maps runtime)\b.{0,160}\b(implemented|implementation complete|created|exists|ships|claimed)\b|\b(implements|implemented|created|ships)\b.{0,80}\b(audit map(?! contract)|audit maps(?! contract))\b'
    ...
    Assert-NoForbiddenPositiveClaim -Text $r16CurrentText -Context "Status docs" -ClaimLabel "context-load planner implementation" -Pattern '(?i)\b(context-load planner|context load planner|context-load plan|context load plan)\b.{0,160}\b(implemented|implementation complete|created|exists|ships|runtime)\b|\b(implements|implemented|created|ships)\b.{0,80}\b(context-load planner|context load planner|context-load plan|context load plan)\b'
    Assert-NoForbiddenPositiveClaim -Text $r16CurrentText -Context "Status docs" -ClaimLabel "role-run envelope or handoff packet implementation" -Pattern '(?i)\b(role-run envelope|role-run envelopes|role run envelope|role run envelopes|handoff packet|handoff packets)\b.{0,160}\b(implemented|implementation complete|created|exist|exists|ships|runtime)\b'
    Assert-NoForbiddenPositiveClaim -Text $r16CurrentText -Context "Status docs" -ClaimLabel "RACI transition gate or workflow drill implementation" -Pattern '(?i)\b(RACI transition gate|RACI transition gates|workflow drill|workflow drills)\b.{0,160}\b(implemented|implementation complete|created|exists|ships|runtime|ran)\b'
