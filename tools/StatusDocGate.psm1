...
function Test-R16OpeningStatus {
    ...
    if ($kanbanSnapshot.DoneThrough -ne 18 -or $kanbanSnapshot.PlannedStart -ne 19 -or $kanbanSnapshot.PlannedThrough -ne 26) {
        throw "R16 status must keep R16 active through R16-018 only with R16-019 through R16-026 planned only."
    }
    ...
    Assert-RegexMatch -Text $Texts.Readme -Pattern '`R16 Operational Memory, Artifact Map, and Role-Bound Workflow Foundation`\s+is now active on branch `release/r16-operational-memory-artifact-map-role-workflow-foundation` through `R16-018` only' -Message "README must declare R16 active on the R16 branch through R16-018 only."
    Assert-RegexMatch -Text $Texts.ActiveState -Pattern '## Active Milestone\s+`R16 Operational Memory, Artifact Map, and Role-Bound Workflow Foundation`\s+is now active in repo truth through `R16-018` only\.' -Message "ACTIVE_STATE must declare R16 as the active milestone through R16-018 only."
    ...
    Assert-RegexMatch -Text $Texts.R16Authority -Pattern '\*\*Milestone status:\*\*\s+Active in repo truth through `R16-018` only' -Message "R16 authority must declare R16 active through R16-018 only."
    ...
    Assert-RegexMatch -Text $Texts.R16Authority -Pattern 'R16-019` through `R16-026` remain planned only' -Message "R16 authority must keep R16-019 through R16-026 planned only."
    ...
    Assert-RegexMatch -Text $r16CurrentText -Pattern 'No generated role-run envelopes are implemented yet|No generated role-run envelope is implemented yet|No generated role-run envelopes exist yet|No generated role-run envelope exists yet' -Message "Status docs must state that no generated role-run envelopes exist yet."
    Assert-RegexMatch -Text $r16CurrentText -Pattern 'No role-run envelope generator exists yet' -Message "Status docs must state that no role-run envelope generator exists yet."
    ...
    Assert-NoForbiddenPositiveClaim -Text $r16CurrentText -Context "Status docs" -ClaimLabel "R16-019 or later implementation" -Pattern '(?i)\bR16-(0(?:1[9]|2[0-6]))\b.{0,160}\b(done|complete|completed|implemented|executed|ran|claimed|created)\b'
    ...
    Assert-NoForbiddenPositiveClaim -Text $r16CurrentText -Context "Status docs" -ClaimLabel "generated role-run envelope or handoff packet implementation" -Pattern '(?i)\b(generated role-run envelope|generated role-run envelopes|role-run envelope generator|role run envelope generator|handoff packet|handoff packets)\b.{0,160}\b(implemented|implementation complete|created|exist|exists|ships|runtime)\b'
