...
function Assert-R13ActiveStatusDocs {
    ...
    Assert-RegexMatch -Text $Texts.ActiveState -Pattern '## Active Milestone\s+`R13 API-First QA Pipeline and Operator Control-Room Product Slice`\s+is now active in repo truth through `R13-002` only\.' -Message "ACTIVE_STATE must declare R13 active through R13-002 only."
...
function Test-R13OpeningStatus {
    ...
    if ($kanbanSnapshot.DoneThrough -ne 2 -or $kanbanSnapshot.PlannedStart -ne 3 -or $kanbanSnapshot.PlannedThrough -ne 18) {
        throw "R13 status must keep only R13-001 through R13-002 done and R13-003 through R13-018 planned."
    }
    ...
    Assert-RegexMatch -Text $Texts.Readme -Pattern 'R13 API-First QA Pipeline and Operator Control-Room Product Slice`\s+is now the active milestone in repo truth through `R13-002` only' -Message "README must declare R13 active through R13-002 only."
    Assert-R13ActiveStatusDocs -Texts $Texts
    ...
    Assert-RegexMatch -Text $Texts.R13Authority -Pattern 'R13 API-First QA Pipeline and Operator Control-Room Product Slice`\s+is now active in repo truth through `R13-002` only' -Message "R13 authority must declare R13 active through R13-002 only."
    ...
    Assert-RegexMatch -Text $combinedText -Pattern '(?i)R13-003`\s+through\s+`R13-018`\s+remain planned only|R13-003` through `R13-018` remain planned only' -Message "Status docs must keep R13-003 through R13-018 planned only."
