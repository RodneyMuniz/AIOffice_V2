...
Assert-Condition -Condition ($authority -like "*Active through*R18-022*safety, secrets, budget, and token controls foundation only*") -Message "R18 authority must state active through R18-022 only."
Assert-Condition -Condition ($authority -like "*R18-023*R18-028*planned only*") -Message "R18 authority must state R18-023 through R18-028 planned only."
...
Write-Output "R18 opening authority state remains active through R18-001 only; current status is active through R18-022 only with R18-023 through R18-028 planned only."
