...
function Test-R18ApiSafetyControlsStatusTruth {
    ...
    foreach ($taskNumber in 1..28) {
        ...
        if ($taskNumber -le 24) {
            Assert-R18ApiSafetyControlsCondition -Condition ($authorityStatuses[$taskId] -eq "done") -Message "$taskId must be done after R18-024."
        }
        else {
            Assert-R18ApiSafetyControlsCondition -Condition ($authorityStatuses[$taskId] -eq "planned") -Message "$taskId must remain planned only after R18-024."
        }
    }
