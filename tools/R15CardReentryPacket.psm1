...
function Assert-R15CardReentryPacketStatusPosture {
    ...
    foreach ($taskId in @("R15-001", "R15-002", "R15-003", "R15-004", "R15-005", "R15-006", "R15-007")) {
        if ($kanbanStatus[$taskId] -ne "done" -or $authorityStatus[$taskId] -ne "done") {
            throw "R15 status posture must mark $taskId done for R15-007."
        }
    }
    ...
    Assert-NoForbiddenPositiveClaim -Text $combinedText -Context "R15-007 status docs" -ClaimLabel "R15-008 or later completion" -Pattern '(?i)\bR15-00[8-9]\b.{0,160}\b(done|complete|completed|implemented|executed|ran)\b'
