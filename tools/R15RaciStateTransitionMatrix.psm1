...
function Assert-R15RaciStateTransitionStatusPosture {
    ...
    if ($kanbanStatus["R15-009"] -notin @("planned", "done") -or $authorityStatus["R15-009"] -notin @("planned", "done")) {
        throw "R15 status posture must keep R15-009 planned at the R15-006 boundary or done at the R15-009 boundary."
    }

    $combinedText = [string]::Join([Environment]::NewLine, @($texts.Values))
    Assert-RegexMatch -Text $combinedText -Pattern '(?i)R15 active through `?R15-006`? only|R15 active through `?R15-008`? only|Active in repo truth through `R15-008` only|through `R15-008` only|R15.*complete through `?R15-009`?.*pending external audit/review|R15 active through R15-009 only|Active in repo truth through `R15-009` only' -Message "Status docs must state either the R15-006/R15-008 boundary posture or the current R15-009 pending external review posture."
    ...
    Assert-NoForbiddenPositiveClaim -Text $combinedText -Context "R15-006 status docs" -ClaimLabel "R15 implementation beyond R15-009" -Pattern '(?i)\b(R15-010|R15 successor task)\b.{0,160}\b(done|complete|completed|implemented|executed|ran|exists|created|planned)\b'
    Assert-NoForbiddenPositiveClaim -Text $combinedText -Context "R15-006 status docs" -ClaimLabel "R15 external audit acceptance" -Pattern '(?i)\bR15\b.{0,160}\b(externally accepted|external audit accepted|external acceptance)\b|\bexternal audit accepted\b'
    Assert-NoForbiddenPositiveClaim -Text $combinedText -Context "R15-006 status docs" -ClaimLabel "R15 main merge" -Pattern '(?i)\bR15\b.{0,160}\b(merged to main|main merge exists|main merged)\b'
