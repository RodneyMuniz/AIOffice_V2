...
function Test-R18Cycle4AuditCloseoutHarnessStatusTruth {
    ...
    foreach ($taskNumber in 1..28) {
        ...
        if ($taskNumber -le 26) {
            Assert-R18Cycle4AuditCloseoutHarnessCondition -Condition ($authorityStatuses[$taskId] -eq "done") -Message "$taskId must be done after R18-026."
        }
        else {
            Assert-R18Cycle4AuditCloseoutHarnessCondition -Condition ($authorityStatuses[$taskId] -eq "planned") -Message "$taskId must remain planned only after R18-026."
        }
    }
