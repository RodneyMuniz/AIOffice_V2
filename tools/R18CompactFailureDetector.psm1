...
function Test-R18CompactFailureDetectorStatusTruth {
    ...
    foreach ($taskNumber in 1..28) {
        ...
        if ($taskNumber -le 14) {
            Assert-R18CompactCondition -Condition ($authorityStatuses[$taskId] -eq "done") -Message "$taskId must be done after R18-014."
        }
        else {
            Assert-R18CompactCondition -Condition ($authorityStatuses[$taskId] -eq "planned") -Message "$taskId must remain planned only after R18-014."
        }
    }
