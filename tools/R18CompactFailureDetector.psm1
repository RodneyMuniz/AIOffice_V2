...
function Test-R18CompactFailureDetectorStatusTruth {
    ...
    foreach ($taskNumber in 1..28) {
        ...
        if ($taskNumber -le 15) {
            Assert-R18CompactCondition -Condition ($authorityStatuses[$taskId] -eq "done") -Message "$taskId must be done after R18-015."
        }
        else {
            Assert-R18CompactCondition -Condition ($authorityStatuses[$taskId] -eq "planned") -Message "$taskId must remain planned only after R18-015."
        }
    }
