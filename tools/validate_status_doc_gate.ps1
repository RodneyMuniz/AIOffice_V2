...
if ($validation.R18Opened) {
    Write-Output ("VALID: status-doc gate records R17 accepted and closed with caveats through R17-{0} only, active milestone '{1}' through R18-{2}, R18-{3} through R18-{4} planned only, R18-002 through R18-012 bounded foundations, R18-013 continuation packet generator as deterministic packet-artifact foundation only, R18-014 new-context prompt generator as deterministic text-artifact foundation only, continuation packets not executed, prompt packets not executed, no automatic new-thread creation, no Codex thread creation, no Codex/OpenAI API invocation, no recovery action, no retry execution, no WIP cleanup/abandonment, no branch mutation or pull/rebase/reset/merge/checkout/switch/clean/restore, no stage/commit/push by generator, no work orders executed, no board/card runtime mutation, no A2A messages sent, no live agents or skills invoked, no recovery runtime, no product runtime, no solved Codex compaction/reliability, no no-manual-prompt-transfer success, and no main merge." -f $validation.R17DoneThrough.ToString("000"), $validation.ActiveMilestone, $validation.R18DoneThrough.ToString("000"), $validation.R18PlannedStart.ToString("000"), $validation.R18PlannedThrough.ToString("000"))
}
