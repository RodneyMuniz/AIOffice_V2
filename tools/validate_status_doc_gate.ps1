...
if ($validation.R16Opened) {
    ...
    Write-Output ("VALID: status-doc gate records R8 closed with tasks through R8-{0} complete, most recently closed milestone '{1}', R10 through R10-{2} closed, R11 through R11-{3} closed, R12 through R12-{4} closed, R13 failed/partial through R13-{5} only, R14 accepted with caveats through R14-{6}, R15 accepted with caveats through R15-{7}, and active milestone '{8}' through R16-{9} with {10}; R16-005-or-later implementation, target-as-achieved KPI scoring, R16-027 or later tasks, R16 closure, R13 closure, R14/R15 caveat-removal, R13 partial-gate conversion, runtime, product UI, agent, memory-layer, artifact-map, role-run-envelope, integration, retrieval/vector, persistent-memory runtime, main-merge, and solved-Codex claims are rejected." -f $validation.DoneThrough.ToString("000"), $validation.MostRecentlyClosedMilestone, $validation.R10DoneThrough.ToString("000"), $validation.R11DoneThrough.ToString("000"), $validation.R12DoneThrough.ToString("000"), $validation.R13DoneThrough.ToString("000"), $validation.R14DoneThrough.ToString("000"), $validation.R15DoneThrough.ToString("000"), $validation.ActiveMilestone, $validation.R16DoneThrough.ToString("000"), $r16PlannedSummary)
}
