...
if ($validation.R18Opened) {
    Write-Output ("VALID: status-doc gate records R17 accepted and closed with caveats through R17-{0} only, active milestone '{1}' through R18-{2}, R18-{3} through R18-{4} planned only, R18-002 agent cards as schema/seed governance artifacts only, R18-003 skill contracts as schema/seed governance artifacts only, R18-004 A2A handoff packets as schema/seed governance artifacts only, R18-005 role-to-skill permission matrix as governance/control artifact only, R18-006 Orchestrator control intake as contract/seed packets only, R18-007 local runner CLI shell as dry-run-only foundation, R18-008 work-order state machine as foundation only, R18-009 runner state store and resumable execution log as deterministic foundation only, R18-010 compact failure detector as deterministic seed-signal foundation only, R18-011 WIP classifier as deterministic seed git-inventory classification foundation only, R18-012 remote branch verifier as bounded branch/head/tree/remote-head foundation evidence only, R18-013 continuation packet generator as deterministic packet-artifact foundation only, current branch identity verified only by bounded git identity checks, permission matrix not runtime enforcement, intake packets not live chat UI or Orchestrator runtime, CLI shell not full work-order execution runtime, work-order state machine not runtime execution, runner state store not live runner runtime, execution log not live execution evidence, resume checkpoint not a continuation packet, failure events not recovery completion, no WIP cleanup/abandonment/restore/delete or classifier stage/commit/push, no verifier branch mutation or pull/rebase/reset/merge/checkout/switch/clean/restore/stage/commit/push, continuation packets not executed, new-context prompt generator not implemented, automatic new-thread creation not implemented, no recovery action performed, no retry execution performed, no work orders executed, handoff packets not live A2A runtime, no board/card runtime mutation, no A2A messages sent, no live agents invoked, no live skills executed, R18 runtime implementation not yet delivered, no API invocation, no live recovery runtime, no live A2A runtime, no local runner runtime execution, no stage/commit/push by detector or generator, no product runtime, no solved Codex compaction/reliability, no no-manual-prompt-transfer success, and no main merge." -f $validation.R17DoneThrough.ToString("000"), $validation.ActiveMilestone, $validation.R18DoneThrough.ToString("000"), $validation.R18PlannedStart.ToString("000"), $validation.R18PlannedThrough.ToString("000"))
}
