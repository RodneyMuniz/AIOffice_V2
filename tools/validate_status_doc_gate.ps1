...
if ($validation.R18Opened) {
    Write-Output ("VALID: status-doc gate records R17 accepted and closed with caveats through R17-{0} only, active milestone '{1}' through R18-{2}, R18-028 final proof package and acceptance recommendation candidate only, no R19 opened, CI replay gap disclosed, no operator approval, no external audit acceptance, no milestone closeout, no closeout without operator approval, no live wrapper/runtime board execution, no release gate execution, no CI replay, no automatic new-thread creation, no Codex thread creation, no Codex/OpenAI API invocation, no recovery action, no WIP cleanup/abandonment, no branch mutation or pull/rebase/reset/merge/checkout/switch/clean/restore, no board/card runtime mutation, no A2A messages sent, no live agents or skills invoked, no recovery runtime, no product runtime, no solved Codex compaction/reliability, no no-manual-prompt-transfer success, and no main merge." -f $validation.R17DoneThrough.ToString("000"), $validation.ActiveMilestone, $validation.R18DoneThrough.ToString("000"))
}
