...
if ($validation.R18Opened) {
    Write-Output ("VALID: status-doc gate records R17 accepted and closed with caveats through R17-{0} only, active milestone '{1}' through R18-{2}, R18-{3} through R18-{4} planned only, R18-002 agent cards as schema/seed governance artifacts only, R18-003 skill contracts as schema/seed governance artifacts only, skill contracts not live skill execution, R18 runtime implementation not yet delivered, no A2A handoff schema, no API invocation, no live recovery runtime, no live A2A runtime, no local runner runtime, no automatic new-thread creation, no product runtime, no solved Codex compaction/reliability, no no-manual-prompt-transfer success, and no main merge." -f $validation.R17DoneThrough.ToString("000"), $validation.ActiveMilestone, $validation.R18DoneThrough.ToString("000"), $validation.R18PlannedStart.ToString("000"), $validation.R18PlannedThrough.ToString("000"))
}
