...
if ($validation.R15Opened) {
    ...
    Write-Output ("VALID: status-doc gate records R8 closed with tasks through R8-{0} complete, most recently closed milestone '{1}', R10 through R10-{2} closed, R11 through R11-{3} closed, R12 through R12-{4} closed, R13 failed/partial through R13-{5} only, R14 accepted/narrowly complete through R14-{6}, and active milestone '{7}' through R15-{8} with {9}, pending external audit/review, and no R16 opening." -f $validation.DoneThrough.ToString("000"), $validation.MostRecentlyClosedMilestone, $validation.R10DoneThrough.ToString("000"), $validation.R11DoneThrough.ToString("000"), $validation.R12DoneThrough.ToString("000"), $validation.R13DoneThrough.ToString("000"), $validation.R14DoneThrough.ToString("000"), $validation.ActiveMilestone, $validation.R15DoneThrough.ToString("000"), $r15PlannedSummary)
}
