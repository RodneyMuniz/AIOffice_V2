...
if ($validation.R18Opened) {
    Write-Output ("VALID: status-doc gate records R17 accepted and closed with caveats through R17-{0} only, active milestone '{1}' through R18-{2}, R18-{3} through R18-{4} planned only, R18-002 agent cards as schema/seed governance artifacts only, R18-003 skill contracts as schema/seed governance artifacts only, R18-004 A2A handoff packets as schema/seed governance artifacts only, R18-005 role-to-skill permission matrix as governance/control artifact only, R18-006 Orchestrator control intake as contract/seed packets only, R18-007 local runner CLI shell as dry-run-only foundation, permission matrix not runtime enforcement, intake packets not live chat UI or Orchestrator runtime, CLI shell not full work-order execution runtime, work-order state machine not implemented, no work orders executed, handoff packets not live A2A runtime, no board/card runtime mutation, no A2A messages sent, no live agents invoked, no live skills executed, R18 runtime implementation not yet delivered, no API invocation, no live recovery runtime, no live A2A runtime, no local runner runtime execution, no automatic new-thread creation, no stage/commit/push by runner, no product runtime, no solved Codex compaction/reliability, no no-manual-prompt-transfer success, and no main merge." -f $validation.R17DoneThrough.ToString("000"), $validation.ActiveMilestone, $validation.R18DoneThrough.ToString("000"), $validation.R18PlannedStart.ToString("000"), $validation.R18PlannedThrough.ToString("000"))
}
