...
if ($validation.R18Opened) {
    Write-Output ("VALID: status-doc gate records R17 accepted and closed with caveats through R17-{0} only, active milestone '{1}' through R18-{2}, R18-{3} planned only, R18-002 through R18-012 bounded foundations, R18-013 continuation packet generator as deterministic packet-artifact foundation only, R18-014 new-context prompt generator as deterministic text-artifact foundation only, R18-015 retry/escalation policy as deterministic policy-artifact foundation only, R18-016 operator approval gate model as deterministic governance-artifact foundation only, R18-017 stage/commit/push gate foundation only, R18-018 status-doc gate wrapper foundation only, R18-019 evidence package wrapper foundation only, R18-020 board/card event model foundation only, R18-021 agent invocation/tool-call evidence model foundation only, R18-022 safety/secrets/budget/token controls foundation only, R18-023 optional API adapter stub foundation only, R18-024 compact-failure recovery drill foundation only, R18-025 Cycle 3 QA/fix-loop harness evidence package only, R18-026 Cycle 4 audit/closeout harness evidence package only, R18-027 operator burden reduction metrics foundation only, CI replay gap disclosed, no external audit acceptance, no milestone closeout, no closeout without operator approval, no live wrapper/runtime board execution, no release gate execution, no CI replay, no automatic new-thread creation, no Codex thread creation, no Codex/OpenAI API invocation, no recovery action, no WIP cleanup/abandonment, no branch mutation or pull/rebase/reset/merge/checkout/switch/clean/restore, no board/card runtime mutation, no A2A messages sent, no live agents or skills invoked, no recovery runtime, no product runtime, no solved Codex compaction/reliability, no no-manual-prompt-transfer success, and no main merge." -f $validation.R17DoneThrough.ToString("000"), $validation.ActiveMilestone, $validation.R18DoneThrough.ToString("000"), $validation.R18PlannedStart.ToString("000"))
}
