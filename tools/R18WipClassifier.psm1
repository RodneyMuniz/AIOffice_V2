...
function Test-R18WipClassifierStatusTruth {
    ...
    foreach ($taskNumber in 1..28) {
        ...
        if ($taskNumber -le 16) {
            Assert-R18WipCondition -Condition ($authorityStatuses[$taskId] -eq "done") -Message "$taskId must be done after R18-016."
        }
        else {
            Assert-R18WipCondition -Condition ($authorityStatuses[$taskId] -eq "planned") -Message "$taskId must remain planned only after R18-016."
        }
    }
