...
function Test-R18OperatorBurdenReductionMetricsStatusTruth {
    ...
    foreach ($taskNumber in 1..28) {
        ...
        if ($taskNumber -le 27) {
            Assert-R18OperatorBurdenReductionMetricsCondition -Condition ($authorityStatuses[$taskId] -eq "done") -Message "$taskId must be done after R18-027."
        }
        else {
            Assert-R18OperatorBurdenReductionMetricsCondition -Condition ($authorityStatuses[$taskId] -eq "planned") -Message "$taskId must remain planned only after R18-027."
        }
    }
