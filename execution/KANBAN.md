...
- Milestone: `R13 API-First QA Pipeline and Operator Control-Room Product Slice`
- Depends on: `R13-010`
- Authority: external replay request/result/import contracts, blocked dispatch evidence, and manual dispatch packet
- Durable output: exact request head/tree, blocked dispatch command results, manual dispatch packet, raw logs, and validation manifest
- Done when: actual external run evidence exists, or unavailable dispatch fails closed without claiming external proof. R13-011 is done in the fail-closed blocked posture; no external proof is claimed.
