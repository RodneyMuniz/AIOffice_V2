...
`R15 Knowledge Base, Agent Identity, Memory, and RACI Foundations` is now active on branch `release/r15-knowledge-base-agent-identity-memory-raci-foundations` through `R15-008` only. R15 opens from R14 head `43653f3dd2e18b46c9e7b02f0c9c095848aee6fc` and locally observed R14 tree `2af1a4aaa858af315e9b4d106d0643b5ce4ebfcc`. R15-009 is planned only.
...
Current R15 status: R15 active through R15-008 only. R15-009 is planned only. R14 remains accepted narrowly through R14-006 only.
...
- Done when: agent memory scope validation and focused tests pass, status surfaces record R15 active through R15-005 only, R15-006 through R15-009 remain planned only, and no agent runtime, persistent memory engine, runtime memory loading, retrieval, vector search, RACI matrix, card re-entry packet, integration, product runtime, or R16 opening is claimed
...
- Done when: RACI and state-transition validation and focused tests pass, the R15-006 boundary is recorded without agent runtime, board routing runtime, PM automation, actual workflow execution, card re-entry runtime, integration, product runtime, or R16 opening claims, and current status surfaces preserve R15-008 through R15-009 planned only after R15-007
...
- Done when: card re-entry packet validation and focused tests pass, status surfaces record R15 active through R15-007 only, R15-008 through R15-009 remain planned only, and no agent runtime, memory runtime, retrieval engine, vector search, board routing runtime, card re-entry runtime, dry run, product runtime, integration, or R16 opening is claimed
...
- Done when: one bounded R14 evidence slice is classified with exact knowledge-index refs, evidence-auditor role selection, memory-scope constraints, RACI transition constraints, a model-only card re-entry packet output, and explicit dry-run/runtime distinction, while R15-009 remains planned only
...
- Done when: planned only after R15-008
