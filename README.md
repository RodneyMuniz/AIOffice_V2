...
`R12 External API Runner, Actionable QA, and Operator Control-Room Workflow Pilot` is now the active milestone in repo truth through `R12-013` only on branch `release/r12-external-api-runner-actionable-qa-control-room-pilot`. R12 opens only after verifying R11 final accepted closeout head `c3bcdf803c0370db66eaa0a9227b3c2301b28fa2`, the R11 audit/R12 planning report commit `5aa08904b02663a5549d2c8a21971544476ae805`, and the planning-report tree `ac324d20d4538e50bfdcb92fe192185a824a2f48`. The report at `governance/reports/AIOffice_V2_R11_Audit_and_R12_Planning_Report_v1.md` is a narrative planning artifact only; it is not milestone proof and does not widen R11. `R12-001` through `R12-013` are done as bounded foundations: value gates are frozen; scorecard, operating-loop, remote-head/stale-phase, fresh-thread bootstrap, transition residue preflight, external runner contract/tooling, bounded replay workflow/bundle, external artifact evidence normalization, actionable QA report, actionable QA fix queue, and cycle QA evidence gate contract/tooling foundations are present. R12-011 through R12-013 add `contracts/actionable_qa/actionable_qa_report.contract.json`, `contracts/actionable_qa/actionable_qa_issue.contract.json`, `tools/ActionableQa.psm1`, `tools/invoke_actionable_qa.ps1`, `contracts/actionable_qa/actionable_qa_fix_queue.contract.json`, `tools/ActionableQaFixQueue.psm1`, `tools/export_actionable_qa_fix_queue.ps1`, `contracts/actionable_qa/cycle_qa_evidence_gate.contract.json`, `tools/ActionableQaEvidenceGate.psm1`, `tools/invoke_actionable_qa_evidence_gate.ps1`, fixtures, and focused tests `tests/test_actionable_qa.ps1`, `tests/test_actionable_qa_fix_queue.ps1`, and `tests/test_actionable_qa_evidence_gate.ps1`. `R12-014` through `R12-021` remain planned only. R12 cannot close without all four value gates: external/API runner evidence, actionable QA reports and fix queues tied to real evidence, operator control-room status views, and one real useful build/change cycle. The current real QA evidence gate cannot pass without real external runner result and external artifact evidence. R12 through `R12-013` does not claim final-state replay, R12 closeout, a 10 percent or larger corrected progress uplift, operator control-room delivery, a real build/change gate, production runtime, real production QA, productized control-room behavior, broad CI/product coverage, broad autonomy, solved Codex reliability, or any R13/successor opening.
