...
`R12 External API Runner, Actionable QA, and Operator Control-Room Workflow Pilot` is now the active milestone in repo truth through `R12-017` only on branch `release/r12-external-api-runner-actionable-qa-control-room-pilot`. R12 opens only after verifying R11 final accepted closeout head `c3bcdf803c0370db66eaa0a9227b3c2301b28fa2`, the R11 audit/R12 planning report commit `5aa08904b02663a5549d2c8a21971544476ae805`, and the planning-report tree `ac324d20d4538e50bfdcb92fe192185a824a2f48`. The report at `governance/reports/AIOffice_V2_R11_Audit_and_R12_Planning_Report_v1.md` is a narrative planning artifact only; it is not milestone proof and does not widen R11. `R12-001` through `R12-017` are done as bounded foundations plus one useful executable refresh workflow: value gates are frozen; scorecard, operating-loop, remote-head/stale-phase, fresh-thread bootstrap, transition residue preflight, external runner contract/tooling, bounded replay workflow/bundle, external artifact evidence normalization, actionable QA report, actionable QA fix queue, cycle QA evidence gate, static control-room status model, Markdown control-room view, operator decision queue, and one-command control-room refresh tooling are present. R12-017 adds `contracts/control_room/control_room_refresh_result.contract.json`, `tools/ControlRoomRefresh.psm1`, `tools/refresh_control_room.ps1`, `tests/test_control_room_refresh.ps1`, fixtures under `state/fixtures/valid/control_room_refresh/` and `state/fixtures/invalid/control_room_refresh/`, current refresh artifact `state/control_room/r12_current/control_room_refresh_result.json`, refreshed current control-room artifacts, and bounded cycle evidence under `state/cycles/r12_real_build_cycle/`. `R12-018` through `R12-021` remain planned only. R12 cannot close without all four value gates: external/API runner evidence, actionable QA reports and fix queues tied to real evidence, operator-readable control-room evidence, and one real useful build/change cycle. The current real QA evidence gate cannot pass without real external runner result and external artifact evidence. R12 through `R12-017` does not claim R12-018 completion, final-state replay, R12 closeout, a 10 percent or larger corrected progress uplift, productized control-room behavior, a full UI app, production runtime, real production QA, broad CI/product coverage, broad autonomy, solved Codex reliability, or any R13/successor opening.
