...
`R7 Fault-Managed Continuity and Rollback Drill` is now the active milestone in repo truth, opened through `R7-001` as bounded structure only. R7 exists because the honestly closed R6 delivery still exposed the next trust weakness: milestone work must survive real interruption and context-window breaks across governed segments without depending on narrative reconstruction, and any rollback drill must stay explicitly governed, disposable, and operator-approved before Git mutation. The next gated step is `R7-002 Add first-class fault / interruption event contracts`. Opening R7 does not yet prove fault-managed continuity, supervised resume-from-fault behavior, rollback execution, unattended automatic resume, UI, Standard runtime, multi-repo behavior, swarms, or broader orchestration.
