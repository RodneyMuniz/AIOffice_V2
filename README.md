...
`R9-001` is currently backed by status-surface validation only:
- `powershell -NoProfile -ExecutionPolicy Bypass -File tests\test_status_doc_gate.ps1`
- `powershell -NoProfile -ExecutionPolicy Bypass -File tools\validate_status_doc_gate.ps1`

This opening validation proves only that R9 is active through `R9-001`, R8 remains the most recently closed milestone, R9 tasks after `R9-001` remain planned only, and the required R9 non-claims are present. It does not implement isolated QA signoff, exact-final post-push support packets, external runner proof, continuity segment artifacts, or the tiny milestone pilot.
