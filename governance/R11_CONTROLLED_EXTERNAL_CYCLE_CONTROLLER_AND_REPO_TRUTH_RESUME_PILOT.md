...
## Current status
`R11 Controlled External Cycle Controller and Repo-Truth Resume Pilot` is now closed narrowly in repo truth after `R11-009` Phase 2 post-push final-head support.

`R11-001` through `R11-009` are complete. Phase 1 `R11-009` candidate closeout is prepared under `state/proof_reviews/r11_controlled_external_cycle_controller_and_repo_truth_resume_pilot/`, candidate closeout commit `545232bfd06df86018917bc677e6ba3374b3b9c4`, and Phase 2 post-push final-head support exists under `state/proof_reviews/r11_controlled_external_cycle_controller_and_repo_truth_resume_pilot/final_head_support/final_remote_head_support_packet.json`. This final support evidence is outside the candidate closeout commit it verifies.
...
- Durable output: `contracts/cycle_controller/cycle_qa_gate.contract.json`, `contracts/cycle_controller/cycle_qa_signoff_packet.contract.json`, `state/fixtures/valid/cycle_controller/cycle_qa_signoff_packet.valid.json`, invalid fixtures under `state/fixtures/invalid/cycle_controller/`, `tools/CycleQaGate.psm1`, `tools/invoke_cycle_qa_gate.ps1`, and `tests/test_cycle_qa_gate.ps1`.
- Boundary: this is separate QA gate contracts/tooling over bounded Dev evidence only. It treats Dev result packets as source evidence, not QA authority, and does not run QA over a complete controlled cycle, execute a complete controlled cycle, run real production QA, generate final audit packets, close R11, or open any successor milestone.

### `R11-008` Execute one complete controlled cycle
- Status: done
- Done when: one operator-approved request runs through bounded cycle-control evidence, one Dev dispatch with 2 bounded tasks, Dev result evidence, separate QA, audit packet, and decision packet while reducing manual per-task prompting.
- Durable output: `contracts/cycle_controller/cycle_audit_packet.contract.json`, `contracts/cycle_controller/operator_decision_packet.contract.json`, `state/cycles/r11_008_controlled_cycle_pilot/`, and `tests/test_r11_controlled_cycle_pilot.ps1`.
- Boundary: this is one bounded controlled-cycle pilot only. It records cycle `cycle-r11-008-controlled-cycle-pilot`, `operator_intervention_count` 2, and `manual_bootstrap_count` 0 after initial approval. It does not close R11, open R12 or any successor milestone, claim real production QA, production runtime, UI/control-room productization, Standard runtime, multi-repo orchestration, swarms, broad autonomous milestone execution, unattended automatic resume, solved Codex context compaction, hours-long unattended execution, destructive rollback, broad CI/product coverage, productized control-room behavior, or general Codex reliability.
...
- Phase 1 candidate output: `state/proof_reviews/r11_controlled_external_cycle_controller_and_repo_truth_resume_pilot/`.
- Phase 2 final support output: `state/proof_reviews/r11_controlled_external_cycle_controller_and_repo_truth_resume_pilot/final_head_support/final_remote_head_support_packet.json`.
- Boundary: R11 closes narrowly only after the Phase 2 final-head support packet verifies the pushed candidate closeout commit `545232bfd06df86018917bc677e6ba3374b3b9c4` from outside that candidate commit. The R11 closeout is limited to one bounded controlled-cycle pilot, R11-008 cycle evidence, the R11-009 candidate closeout package, and the R11-009 post-push final-head support packet.
...
- `R11-007` adds separate QA gate contracts/tooling only. It consumes Dev evidence refs, preserves dispatch/result/cycle refs, rejects executor self-certification as QA authority, rejects Dev-result QA authority or QA verdict claims, rejects complete controlled cycle and successor claims, and requires a separate QA actor or explicit non-self-certification independence boundary. It does not run QA over a complete controlled cycle, execute a complete controlled cycle, run real production QA, generate final audit packets, close R11, or open any successor milestone.
- `R11-008` is complete as one bounded controlled-cycle pilot only; `R11-009` is complete as a two-phase narrow closeout with Phase 1 candidate evidence and Phase 2 post-push final-head support.
