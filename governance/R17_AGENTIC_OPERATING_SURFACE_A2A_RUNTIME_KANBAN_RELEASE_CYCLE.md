...
| Phase E | Tool adapters and ledgers | `R17-015` through `R17-019` | `R17-015` done as common tool adapter contract foundation only with disabled seed adapter profiles; `R17-016` through `R17-019` planned only |
...
These cycles are not working yet and are not claimed by `R17-001` through `R17-015`.
...
This R17 active boundary through `R17-015` claims none of the following:
...
- no R17-016 or later implementation yet;
...
- R17-016 through R17-028 are implemented by this pass;
